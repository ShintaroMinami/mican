...
2. [Domain Detection — Check Before Concluding Cn](#2-domain-detection--check-before-concluding-cn)
3. [Rotation Matrix Analysis](#3-rotation-matrix-analysis)
4. [Cyclic Permutation — Algebraic Proof](#4-cyclic-permutation--algebraic-proof)
5. [Pseudo Higher-Order Symmetry](#5-pseudo-higher-order-symmetry)
6. [Distinguishing True Symmetry from False Positives](#6-distinguishing-true-symmetry-from-false-positives)
...
## 2. Domain Detection — Check Before Concluding Cn

> **⚠ Perform this check before interpreting rotation angles or cyclic permutations.**
> If the protein contains multiple structural domains, sub-optimal alignments spanning domain
> boundaries will produce rotation angles that do not fit any Cn pattern. Misidentifying
> these as symmetry operations leads to incorrect conclusions.
...
## 4. Cyclic Permutation — Algebraic Proof

> **Requires:** `get_rot_vec_pairs` and `analyze_rotation` defined in Section 3.
...
def estimate_unit_size(all_pairs_by_rank):
    ...
    IMPORTANT: Pass only Cn-confirmed ranks (those whose angle matches k/n × 360°
    within ~2°). Including pseudo-symmetric ranks (e.g., C8-like ranks in a C4
    protein) can corrupt the estimation because their dominant shifts may be
    smaller than 1 unit (e.g., ranks at ~38.5° in a C4 protein have shift ≈ 0.5×unit
    due to intra-unit partial alignments, not inter-unit steps).

    Strategy: find the rank whose dominant shift is smallest (= 1 unit shift).
    Use that rank's GCD as the unit size.

    Why smallest-shift rank instead of lowest rank number:
    - In C4, Rank 2 (180°) maps each unit to its opposite → shift ≈ 2×unit.
      Rank 3 (90°) maps each unit to its neighbor  → shift ≈ 1×unit.
    - Rank 2 has a smaller rank number but a larger shift.
    - The rank with the smallest dominant shift gives the fundamental unit directly.
    - Mixing multiple ranks (e.g., 2u + u + 3u) collapses GCD to 1 via noise.
    """
    # For each rank, find the dominant (most common) non-zero shift
    rank_dominant = {}
    for rank, pairs in all_pairs_by_rank.items():
        shifts = [abs(r2 - r1) for r1, r2 in pairs if r2 != r1]
        if shifts:
            rank_dominant[rank] = Counter(shifts).most_common(1)[0][0]

    if not rank_dominant:
        return None

    # Use the rank with the smallest dominant shift — that is the 1-unit rank
    best_rank = min(rank_dominant, key=rank_dominant.get)
    dominant = rank_dominant[best_rank]

    # Compute GCD using ONLY the dominant shift value (filter out noisy minority shifts).
    # Using all shifts collapses GCD: e.g., dominant=42 but a few pairs have shift=44
    # gives GCD(42, 44) = 2, which is wrong. Using only the dominant shift is safe.
    pairs = all_pairs_by_rank[best_rank]
    dominant_shifts = [abs(r2 - r1) for r1, r2 in pairs if abs(r2 - r1) == dominant]
    g = reduce(gcd, dominant_shifts) if dominant_shifts else dominant
    if g > 1:
        return g

    # Fallback: the dominant shift of the best rank itself
    return dominant
...
# (uses get_rot_vec_pairs / analyze_rotation from Section 3)
rotation_results = {}
all_pairs = {}
for rank in range(2, 9):
    rot, vec, pairs = get_rot_vec_pairs(pdb, rank)
...
# Step 2: filter out false positives (rotation angle < 20° — see Section 6)
...
## 5. Pseudo Higher-Order Symmetry

> **Prerequisites:** Complete Sections 3 and 4 first. Set `TRUE_N` to the confirmed symmetry
> order before running this block.

When a protein has true Cn symmetry, some sub-optimal alignments may appear to match a
higher-order C(2n) pattern due to structural similarity between non-adjacent units.
This section detects such pseudo-symmetry and confirms the true Cn order.

```python
import numpy as np

# Set TRUE_N to the confirmed Cn order (from Section 3/4 analysis)
TRUE_N = 3  # <-- set to confirmed Cn order (example: C3; replace with your result)
PSEUDO_N = TRUE_N * 2
...
## 6. Distinguishing True Symmetry from False Positives
